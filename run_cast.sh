...
out_file="file://${datadir}/${file_name}.log::S=${file_rotate_time} -f ${file_overlap_time}"
...
mkdir -p ${logdir}
  ...
  case "$2" in
    ...
    ${cast} -in ${!1} -out ${out_tcp} -c ${BASEDIR}/receiver_cfg/um980-base-rtcmoutcom1-binoutcom2.cmd -b 1 -t ${level} -fl ${logdir}/str2str_tcp.log &
    ;;
  ...
  out_file)
    ...
    then
      ...
      ${cast} -in serial://ttyS0:115200:8:n:1 -out ${out_file} -t ${level} -fl ${logdir}/str2str_file.log &
    fi
    ;;
